...
class InteractTest {
    ...
    @BeforeAll
    static void init() {
        Args args = new Args(new String[]{ODDefault.GO_INDEX_2});
        driver = DriverFactory.getDriver(args);
        driver.get(ODDefault.GO_INDEX_2);
        NavInfoParser parser = new NavInfoParser();
        element = NavSupport.getElements(driver,
                        parser.getNavInfo(ODType.GOINDEX.name(), NavType.GoIndex.LIST_VIEW.name()).getCssFileSelector())
                .get(4);
    }
